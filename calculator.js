...
function verify(expression) {
  ...
  while (result.indexOf("(") !== -1) {
    ...
    if (secondBracket === -1) return false;
    let subexpression = result.substring(firstBracket + 1, secondBracket);
    let _result = result;
    if (verify(subexpression)) {
      result =
        _result.substring(0, firstBracket) +
        "0" +
        _result.substring(secondBracket + 1, _result.length);
    } else {
      return false;
    }
  }
  ...
  for (let i = 0; i < result.length; i++) {
    if (isNaN(parseInt(result[i])) && !signs(result[i])) {
      return false;
    }
...
function calculator(expression) {
  ...
  while (result.indexOf("(") !== -1) {
    ...
    let subexpression = result.substring(firstBracket + 1, secondBracket);
    result =
      result.substring(0, firstBracket) +
      calculator(subexpression) +
      result.substring(secondBracket + 1, result.length);
  }
  console.log(result);
